...
public class H2DialectTest extends AbstractDialectTest {
	...
	public void buildCreateSQLTest() {
		...
		Assert.assertEquals("CREATE TABLE IF NOT EXISTS test (id BIGINT GENERATED BY DEFAULT AS IDENTITY, operatorId BIGINT NOT NULL, createTime TIMESTAMP, updateTime TIMESTAMP, PRIMARY KEY (id))", dialect.buildCreateSQL(null, tableName()));
		Assert.assertEquals("CREATE TABLE IF NOT EXISTS test (id BIGINT GENERATED BY DEFAULT AS IDENTITY, column0field0 VARCHAR(512), operatorId BIGINT NOT NULL, createTime TIMESTAMP, updateTime TIMESTAMP, PRIMARY KEY (id))", dialect.buildCreateSQL(fieldInfoMap(), tableName()));
	}
